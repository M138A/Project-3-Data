...
public class TestWindowController implements Initializable {
    ...
    public void getSocialMediaGroup() {
       try {
           ...
           if(toggle.equals("Alles")){
               sql = "";
               sql += "SELECT COUNT(IF(socialmedia='Twitter',1,null)) 'twitter', COUNT(IF(socialmedia='Facebook',1,null)) 'facebook', COUNT(IF(socialmedia='Google',1,null)) 'google' FROM Bericht";
    ...
    public String getPositiveOrNegative() {
        ...
        try {
            ...
            switch (toggle) {
                case "Allebei":
                    return sql += " WHERE" + " " + allebei;
                case "Positief":
                    return sql += " WHERE" + " " + positief;
                default:
                    return sql += " WHERE" + " " + negatief;
            }
        }catch (Exception e){
